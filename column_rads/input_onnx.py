...
with open("column_rads/error_calibration.json", "r") as f:
...
Column_Style = str(input('Column Style = '))
Height = np.float64(input('Height = '))
Width = np.float64(input('Width = '))
Columns = int(input('Columns = '))

# manually binarize Column_Style
if Column_Style == 'Flat':
    Column_Style = 0
elif Column_Style == 'Simple Tube':
    Column_Style = 1



# Load the ONNX model
session = ort.InferenceSession("column_rads/column_model_pipeline.onnx")

# prepare input dictionary
input_dict = {
    'Column_Style': np.array([[Column_Style]], dtype=np.float32),
    'Height': np.array([[Height]], dtype=np.float32),
    'Width': np.array([[Width]], dtype=np.float32),
    'Cols': np.array([[Columns]], dtype=np.float32)
}
...
session = ort.InferenceSession("column_rads/inverse_target_scaler.onnx")
